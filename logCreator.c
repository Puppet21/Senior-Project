...
/*
 * Zero out resets all data points on a failure
 */
...
int main(int argc, char **argv){
  ...
  int timeH, timeM, timeS, volt, poten, pf = 0;
  double water =0;
  ...
  for(x =0;x<14;x++){
    ...
    if( timeCheck(timeH,timeM,timeS) != true){
      printf("time log was incorrect. Data cannot be used.\n Check to see if the device is damaged!\n");
    }
    if( voltCheck(volt) == -1){
      printf("Error on voltage reading.\n Check to see if the device is damaged.\n");
    }
    if(potenCheck(poten) == -1){
      printf("Error on potentiameter.\n Replace drink cap.\n");
    }
    if(waterCheck(water) == -1){
      printf("Error on water.\n Cup damaged, replace soon.\n");
    }
    if(pf ==1){
      printf("Purge flag found.\n");
    }
  }
